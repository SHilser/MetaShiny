...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Prepare File",
       sidebarLayout( 
         sidebarPanel(
           ...
           ,
           actionButton("showColumns", "Show File and Choose Columns"),
   ...
   ### START: SHOW PDF from Sci-Hub based on DOI from CSV-file (WORKING!!!), maybe adapt to University library #### ####
    ,
    tabPanel("SciHub",
             sidebarLayout(
               sidebarPanel(
   ...
   ### END:   SHOW PDF from Sci-Hub based on DOI from CSV-file (WORKING!!!), maybe adapt to University library ####
   ...
   tabPanel("Dynamic UI",
            sidebarLayout(
              sidebarPanel(
                ...
                radioButtons(inputId = "uiElementType",
                            label = "What type of element do you want to add?",
                            choices = c("Radiobuttons", "Textfield"),
                            inline = TRUE),
                uiOutput("uiElementArguments"),
                # tags$div(id = "uiElementArguments"),
                # textInput("uiLabel", "Choose a label for the element"),
                actionButton("insertElement", "Add New Element"),
                actionButton("removeElement", "Remove")
                # ,
                # uiOutput("tabElements"),
                # tags$div(id = "uiElements"),
   ...
   ### START: User Guide (IN PROGRESS) #### ####
   ,
   tabPanel("User Guide",
            sidebarLayout(
              sidebarPanel(
                HTML("Add a menu that links to the headers, just like the sidebar in Word")
              ),
              mainPanel(
                h3("User Guide"),
                HTML("This will be the user guide that helps you to work with MetaShiny"),
                h4("Load File"),
                HTML("Text"),
                h4("Screen Abstracts"),
                HTML("Text"),
                h4("Download Files"),
                HTML("Text"),
                h4("Full-text review: Create your own User Interface (UI)"),
                HTML("Text")
                )
              )
            )
...
server <- function(input, output){
  ...
  ### START: Insert/Remove fields, used to define review widgets #### ####
      # Insert User Interface Elements
      ## Show fields for relevant options
      
      output$uiElementArguments <- renderUI({
      # observeEvent(input$uiElementType, {
        uiElementLabel <- paste0("label", input$uiElementType, input$insertElement)
        
        switch(input$uiElementType,
               "Radiobuttons" = list(textInput(inputId = uiElementLabel,"Label"),
                                      textInput(inputId = paste0("choices", input$uiElementType, input$insertElement),"Choices")
                                      ),
               "Textfield" = textInput(inputId = uiElementLabel,"Label")
               )
      # )
      })
      ## Create UI Element based on input from previous function ISSUE: Name of already created elements changes when input$uiLabel is changed
      observeEvent(input$insertElement, {
        uiElementId <- paste0(input$uiElementType, input$insertElement)
        uiElementLabel <- paste0("label", input$uiElementType, input$insertElement)  # Add uiElementChoices as soon as this is functioning
        insertUI(
          selector = "#uiElements",
          where = "beforeEnd",
          ui = switch(input$uiElementType,
                      "Radiobuttons" = uiOutput(uiElementId),
                      "Textfield" = uiOutput(uiElementId)
                      )
          )
        output[[uiElementId]] <-
          if (input$uiElementType == "Radiobuttons") renderUI({radioButtons(inputId = uiElementId,
                                                                             label = uiElementLabel,
                                                                             choices = c("Option A", "Option B")
                                                                             )
                                                              })
          else if (input$uiElementType == "Textfield") renderUI({textAreaInput(inputId =  uiElementId,
                                                                               label = uiElementLabel)})
        })
        # Remove User Interface Elements
        observeEvent(input$removeElement, {
          removeUI(
            selector = paste0('div:has(> #', 'uiElements',')')
          )
      })
  ### END:   Insert/Remove fields, used to define review widgets ####
